...
public final class VerifyAccountCommand extends SDLinkSlashCommand {
    ...
    public VerifyAccountCommand() {
        super(false);
        this.name = "verify";
        this.help = Text.translate("command.verify.help").toString();
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if (mcCode == 0) {
            event.getHook().sendMessage(Text.translate("command.verify.missing_code").toString()).setEphemeral(SDLinkConfig.INSTANCE.botConfig.silentReplies).queue();
            return;
        }
        ...
        for (SDLinkAccount account : accounts) {
            ...
            if (accounts.stream().anyMatch(a -> a.getDiscordID() != null && a.getDiscordID().equals(m.getId())) && !SDLinkConfig.INSTANCE.accessControl.allowMultipleAccounts) {
                event.getHook().sendMessage(Text.translate("command.verify.already_verified").toString()).queue();
                return;
            }
        ...
        if (!didVerify)
            event.getHook().sendMessage(Text.translate("command.verify.failed").toString()).setEphemeral(SDLinkConfig.INSTANCE.botConfig.silentReplies).queue();
    }
